...
def plot_results():
    ...
    for idx, row in df.iterrows():
        ...
        local_contacts = np.count_nonzero(local_dist < 1.1 * rod_diameter) / num_rods
    ...
    for i_n,n in enumerate(ns):
        ...
        std = np.std(n,axis=0)
        plt.plot(x_bins[:-1],avg,'-',label=f'{ARs[i_n]}')
        # plt.fill_between(x_bins[:-1], avg-std, avg+std, alpha=0.5)
        
        tmp = []
        for i in range(num_rep):
            dta = n[i]
            popt, pcov = curve_fit(gaussian, x_bins[:-1], dta, p0=[1, 100])
            b = popt[1]
            # goodness of fit
            residuals = dta - gaussian(x_bins[:-1], *popt)
            ss_res = np.sum(residuals**2)
            ss_tot = np.sum((dta - np.mean(dta))**2)
            r_squared = 1 - (ss_res / ss_tot)
            # print(r_squared)

            # plt.plot(x_bins[:-1], gaussian(x_bins[:-1], *popt), '--', color='gray', alpha=0.5)
            sigma = 1/np.sqrt(2*b)
            tmp.append(sigma)
    ...
    plt.savefig('junkyard/packings369_skewness_pdf.png', dpi=600, bbox_inches='tight')
    plt.savefig('junkyard/packings369_skewness_pdf.svg', dpi=300, bbox_inches='tight')

    

    x_fit = np.geomspace(0.01,2,100)
    line1 = x_fit**(-1)
    plt.plot(x_fit,line1,'--',color='gray',alpha=0.5)

    x_fit = np.geomspace(0.1,2,100)
    line2 = 5e-4*x_fit**(-3)
    plt.plot(x_fit,line2,'--',color='gray',alpha=0.5)

    plt.xscale('log')
    plt.yscale('log')
    plt.xlabel('Skewness, $\\Delta_{ij}$')
    plt.ylabel('PDF, $P(\\Delta_{ij})$')
    plt.savefig('junkyard/packings369_skewness_pdf_loglog.png', dpi=600, bbox_inches='tight')
    plt.savefig('junkyard/packings369_skewness_pdf_loglog.svg', dpi=300, bbox_inches='tight')
    ...
    popt,pcov = curve_fit(power_law,ARs,sigmas,p0=[1])
    x_fit = np.linspace(5,500,100)
    ...
    plt.savefig('junkyard/packings369_skewness_width.png', dpi=300, bbox_inches='tight')
    ...
    plt.savefig('junkyard/packings369_distance_pdf.png', dpi=300, bbox_inches='tight')
    ...
    plt.xscale('log')
    plt.yscale('log')
    plt.savefig('junkyard/packings369_distance_pdf_loglog.svg', dpi=300, bbox_inches='tight')
    plt.savefig('junkyard/packings369_distance_pdf_loglog.png', dpi=300, bbox_inches='tight')
    ...
    popt,pcov = curve_fit(exponential_hill,ARs,entanglement_wrt_AR[:,0],p0=[0.5,100])
