...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Run sequential multi-AR relaxation (GPU) starting from a pre-entangled packing."
    )
    ...
    p.add_argument(
        ...
        help="Comma-separated AR values ordered largest→smallest. Default: 1000,500,300,200,100,50",
    )
    ...
    p.add_argument("--resume", action="store_true",
                   help="If set, resume from existing q_relaxed.npy results if found.")
    p.add_argument("--force", action="store_true",
                   help="Recompute even if cached files exist (implies not resuming)")
...
def main() -> None:
    ...
    packing_id = f"{dt_string}_SubsequentRelaxedPacking-N{num_rods}-Scale{scale_factor:g}"
    ...
    print("=" * 60)
    print("  STEP 2: Subsequent Sequential relaxation")
    ...
    for idx, AR in enumerate(ar_list):
        ...
        if args.resume and result_path.exists() and not args.force:
