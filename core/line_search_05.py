...
def create_entangled_rods(initial_q,rod_diameter=0.1,Nmax=1e4,N_outer=1,atol=1e-4,dt=1e-3,callback=None):
    ...
    if isinstance(initial_q, jnp.ndarray):
        q0 = initial_q.flatten()
    
    f = entanglement_potential
    df = grad(f)
    df0 = df(q0)
...
if __name__ == "__main__":
    ...
    initial_q=q.reshape(-1).astype(jnp.float64)
    q_ent = create_entangled_rods(initial_q, rod_diameter=0.01, Nmax=300, N_outer=5, atol=1e-8, dt=1e-3)
    ent2 = get_entanglement(q_ent.reshape(-1,5))
