...
def get_local_dist_mat(svd_cylinders,a_cluster,segments):
    ...
    for i in range(num_elements):
        ...
        p1,p2 = cylinder_i[0:3],cylinder_i[3:6]
        
        fit_result1 = fit_rod(segment_i,0.000001,10000)
        slopes1 = fit_result1['slopes']
        
        for j in range(i+1,num_elements):
            cylinder_j = svd_cylinders[a_cluster[j],:]
            segment_j = segments[a_cluster[j]]
            q1,q2 = cylinder_j[0:3],cylinder_j[3:6]
            
            t,u,d1,d2,d12=lumelsky_dist_vec(p1,p2,q1,q2)
            
            fit_result2 = fit_rod(segment_j,0.000001,10000)
            slopes2 = fit_result2['slopes']
            
            joined = np.vstack([segment_i,segment_j])
            fit_result = fit_rod(joined,0.000001,10000)
            err_factor = fit_result['err']
            ...
            slope1 = slope1 * np.sign( np.dot(slope1,popt1 - popt1_opp) )
            slope2 = slope2 * np.sign( np.dot(slope2,popt2 - popt2_opp) )
            
            dist_vec_sign = -np.sign(np.dot(slope1,slope2))
            dist_vec_sign = 1
            dist = np.linalg.norm(vec)
            local_dist_mat[i,j] = dist*dist_vec_sign*err_factor
            
    local_dist_mat = np.minimum(local_dist_mat,local_dist_mat.T)
    local_dist_mat[local_dist_mat<0] = 1e4
    local_dist_mat[local_dist_mat == np.inf] = 1e4
    return local_dist_mat


def explode_local_cluster(a_cluster,svd_cylinders,segments):
    subgraph = nx.Graph()
    subgraph.add_nodes_from(range(len(a_cluster)))
    # get_local_dist_mat(svd_cylinders,a_cluster,segments)
    local_dist_mat = get_local_dist_mat(svd_cylinders,a_cluster,segments)
    ...
    subclusters = find_connected_components(mst)
...
def do_what():
    ...
    # ax.axis('equal')
def subclustering_by_mst(clusters,segments,svd_cylinders,subcluster_error_threshold,subcluster_length_threshold,subcluster_length_tolerance=50):
    N_segments = len(segments)    
    ...
    for i in range(len(clusters)):
        cluster_length_list[i] = segments_length_list[clusters[i]].sum()
    ...
    for i_ny in not_yet_nodes:
        ...
        if fit_result['err'] < subcluster_error_threshold and (np.abs(rr_len - subcluster_length_threshold)<subcluster_length_tolerance):
    ...
    print(f'Number of good nodes: ', len(good_nodes))
...
def subclustering_by_mst_length_lowerbound(clusters,segments,svd_cylinders,subcluster_error_threshold,subcluster_length_threshold):
    N_segments = len(segments)    
    segments_length_list = np.zeros(N_segments)
    for i,seg in enumerate(segments):
        segments_length_list[i] = np.sum(np.sqrt(np.sum(np.diff(seg,axis=0)**2,axis=1)))
    ...
    for i_cluster, a_cluster in enumerate(clusters):
        ...
        for i_subcluster,subcluster in enumerate(subclusters):
            ...
            fit_result = fit_rod(joined,linearity_threshold=1e-10,radius_curvature_threshold=1e10)
            subcluster_error_list[i_subcluster] = fit_result['err']
            # rec = fit_result['rec']
            # subcluster_length_list[i_subcluster] = segments_length_list[ a_cluster[subcluster] ].sum()
            subcluster_length_list[i_subcluster] = np.sum(np.sqrt(np.sum(np.diff(joined,axis=0)**2,axis=1)))
            
        if (fit_result['planarity'] > 5):
            # local_trash_nodes.update(subclusters[0][0])
            for i_subcluster,subcluster in enumerate(subclusters):
                if subcluster_length_list[i_subcluster] < 10:
                    local_trash_nodes.add(subclusters[i_subcluster][0])
        
        if ((len(subcluster_error_list) == 1) and (subcluster_length_list[0] < 10)):
            local_trash_nodes.add(subclusters[0][0])
        
        certification = (subcluster_error_list < subcluster_error_threshold) & (fit_result['planarity'] < 1) & (subcluster_length_list < 750)
        
        for i_cert in np.where(certification)[0]:
            good_clusters.append(np.array([a_cluster[i] for i in subclusters[i_cert]]))
            
    print(f'[MST] Number of good clusters: ', len(good_clusters))
