...
def create_nonintersecting_random_rods_gpu(num_rods, rod_diameter, max_attempts=10000):
    """GPU-accelerated rod placement using vmap for batched distance checks.
    
    The outer placement loop is sequential (each rod depends on all previous),
    but the inner distance check against all existing rods uses vmap to run
    in parallel on GPU.
    """
    ...
    @jit
    def _check_candidate_against_existing(p_i, p_ii, existing_starts, existing_ends, n_existing):
        """Check candidate rod against all existing rods using vmap."""
        # vmap dist_lin_seg over existing rods (broadcast candidate)
        def _dist_to_one(j_start, j_end):
            return dist_lin_seg(p_i, p_ii, j_start, j_end)
        
        all_dists = vmap(_dist_to_one)(existing_starts, existing_ends)
        # Only consider first n_existing rods (rest are padding zeros)
        # Use a large value for unused slots so they don't trigger intersection
        mask = jnp.arange(existing_starts.shape[0]) < n_existing
        masked_dists = jnp.where(mask, all_dists, jnp.inf)
        return jnp.min(masked_dists)

    print('create_nonintersecting_random_rods_gpu')

    q = onp.zeros((num_rods, 5), dtype=onp.float64)
    # Pre-allocate padded arrays for existing rod endpoints (on GPU)
    ...
    for i in range(num_rods):
        ...
        while not created and attempts < max_attempts:
            ...
            p_i = jnp.array([x, y, z])
            u_i = jnp.array([jnp.sin(phi) * jnp.cos(theta),
                             jnp.sin(phi) * jnp.sin(theta),
                             jnp.cos(phi)])
            p_ii = p_i + u_i

            if i == 0:
                # First rod: no collision check needed
                intersect = False
            else:
                min_dist = _check_candidate_against_existing(
                    p_i, p_ii, existing_starts, existing_ends, i
                )
                if onp.isnan(min_dist):
                    import warnings
                    warnings.warn(f"NaN detected in min_dist at rod {i}, attempt {attempts}")
                    intersect = True # Treat as intersection to retry
                else:
                    intersect = bool(min_dist < rod_diameter)
...
def collision_relaxation(q,f_in,params,N_outer,Nmax,atol,dt,atol_min=1,visualize=False,callback=None):    
    ...
    # Rod diameter is 2*col_rad, and we require min distance >= nextafter(diameter, +inf)
    ...
    def _callback_only_no_overlap(q_state, callback_params):
        # Forward to user callback for logging/side-effects, but ignore its return value
        # so no stopping criterion other than no-overlap can terminate relaxation.
        if callback is not None:
            callback(q_state, callback_params)
        min_d = callback_params.get("min_distance", None)
        if min_d is None:
            return False
        return min_d >= no_overlap_threshold
    ...
    while True:
        ...
        df = jit(grad(jit(f)))
        q, f_val, _, error = optimize_fire_nonjax_individual(
            q,
            f,
            df,
            Nmax,
            atol,
            dt,
            callback=_callback_only_no_overlap,
            allow_callback_break=True,
        )
...
def gpu_relax_collision(q, dt, params, max_iters=1000000, effective_diameter_factor=1.005,
                        log_every=1000):
    """Fully JIT-compiled gradient descent relaxation on GPU.

    Uses lax.while_loop so the entire optimization runs on-device without
    returning to Python.  Converges when min pairwise distance > rod_diameter.
    The repulsion potential uses an effective diameter (effective_diameter_factor * rod_diameter).

    Args:
        q: flattened (N*5,) rod configuration
        dt: gradient descent step size
        params: dict with 'col_rad', 'amp', 'sigma'
        max_iters: safety cap on iterations
        effective_diameter_factor: effective diameter as multiple of rod_diameter for potential
            (default 1.005 = 0.5% clearance)
        log_every: print progress every N iterations (uses host callback)
    """
    from jax import lax
    ...
    print(f"GPU relaxation: rod_diameter={rod_diameter:.6f}, "
          f"effective_diameter={rod_diameter * effective_diameter_factor:.6f}, "
          f"target_min_dist={target_min_dist:.6f}, "
    ...
    # Build the JIT-compiled potential and gradient using EFFECTIVE diameter
    @jit
    ...
    # Compute min pairwise distance (JIT-compiled)
    @jit
    ...
    print(f"  Final min distance: {final_min_d:.10e}")
    print(f"  Target min distance: {target_min_dist:.10e}")
    print(f"  Effective diameter: {rod_diameter * effective_diameter_factor:.10e}")
    print(f"  Rod diameter: {rod_diameter:.10e}")
    print(f"  Converged: {final_min_d >= target_min_dist}")
    print(f"  Wall time: {t1 - t0:.2f}s")
    print(f"  Iters/sec: {n_iters / (t1 - t0):.0f}")
...
def standard_protocol():
    ...
    try:
        ...
        num3 = int(sys.argv[3])
        num4 = int(sys.argv[4])
        print(f"Random keys: {num1}, {num2}, {num3}")
        print(f"AR: {num4}")
    except ValueError:
