...
def setup_directories(script_path, num_rods, aspect_ratio):
    """Creates output directories for simulation results and movies."""
    script_name = Path(script_path).stem
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    
    # Create parent directory named after the script
    parent_dir = Path(script_name)
    parent_dir.mkdir(exist_ok=True)

    # Create run-specific subdirectory
    dir_name = f"N{num_rods}_AR{aspect_ratio}_{timestamp}"
...
def main():
    ...
    # --- Setup ---
    output_dir, movie_dir = setup_directories(__file__, NUM_RODS, ASPECT_RATIO)
    ...
    if system_name == "Darwin":
        movie_path = output_dir / "simulation_movie.mp4"
        ffmpeg_cmd = f"ffmpeg -y -framerate 30 -i {movie_dir}/step-%04d.png -c:v libx264 -pix_fmt yuv420p {movie_path}"
        os.system(ffmpeg_cmd)
        print(f"✅ Movie created at {movie_path}")

        # Cleanup frames
        shutil.rmtree(movie_dir)
        print(f"🗑️ Deleted frame images in {movie_dir}")
