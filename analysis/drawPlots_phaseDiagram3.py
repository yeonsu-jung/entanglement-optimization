...
for dc in data_container_list:
    ...
    static = g/0.5
    dynamic = 4*np.pi**2*freq**2*0.001/0.5
...
# plt.tight_layout()
output_dir = '/Users/yeonsu/Dropbox (Harvard University)/Data/PrunedData/rod-sim-pnas-revision/entanglement'
plt.savefig(f'{output_dir}/entanglement_vs_a-g-cv-3d-with-data2.svg')
...
df_ = df[(df['static'] == 20) & (df['AR'] == 150)]
ax.plot(df_['dynamic'],df_['f'],'s--',label=r'$\alpha=150, \tilde{F} = 20$',markersize=3,color=default_color_list[2],alpha=0.5)
...
marker_size = 10
# [static,mu_over_sigma,dynamic]
static = entangled_data[:,0]
mu_over_sigma = entangled_data[:,1]
dynamic = entangled_data[:,2]
...
# experiment
sim_ag = 4*np.pi**2*(10)**2*0.001/0.5
exp_ag = 2.5

import pandas as pd
df = pd.read_csv('/Users/yeonsu/GitHub/entanglement-source-codes/figure-scripts/PhysicalPacking_moments.csv')

num_untangled = len(df[df['alpha'] < 110]['coefficient_of_variation'])
num_entangled = len(df[df['alpha'] > 110]['coefficient_of_variation'])


ax.scatter(exp_ag*np.ones(num_entangled),df[df['alpha'] > 110]['coefficient_of_variation'],color='b',marker='s',s=marker_size,linewidth=0.5,label='Entangled stiff rods (exp)',facecolor='none')

# previous studies

df_worm = pd.read_csv('/Users/yeonsu/GitHub/entanglement-source-codes/figure-scripts/worm_data_analysis.csv')

xx = sim_ag*np.ones(1)
yy = df_worm['Coefficient of Variation'][0]
ax.scatter(xx,yy,color='r',marker='^',s=marker_size,linewidth=0.5,label='Worms (from Patil et al. Science 2023)',facecolor='none')

xx = sim_ag*np.ones(4)
yy = df_worm['Coefficient of Variation'][1:]
ax.scatter(xx,yy,color='b',marker='^',s=marker_size,linewidth=0.5,facecolor='none',label='Worms (from Patil et al. Science 2023)')



df_gripper = pd.read_csv('/Users/yeonsu/GitHub/entanglement-source-codes/figure-scripts/gripper_data_analysis.csv')
ax.scatter(sim_ag*np.ones(len(df_gripper)),df_gripper['Coefficient of Variation'],color='b',marker='<',s=marker_size,linewidth=0.5,label='Grippers (from Becker et al. PNAS 2022)',facecolor='none')

# ax.scatter(sim_ag,2.0736368597372756,color='b',marker='d',s=marker_size,linewidth=0.5,label='Pigeon nest',facecolor='none')

# legend outside
ax.scatter(exp_ag*np.ones(num_untangled),df[df['alpha'] < 110]['coefficient_of_variation'],color='r',marker='>',s=marker_size,linewidth=0.5,label='Untangled stiff rods (exp)',facecolor='none')
ax.scatter(untangled_data[:,2],1/untangled_data[:,1],color='r',s=marker_size,marker='v',linewidth=0.5,label='Untangled stiff rods (sim)',facecolor='none')
ax.set_yscale('log')
...
# ax.scatter(2.5,1.25,color='k',marker='o')
