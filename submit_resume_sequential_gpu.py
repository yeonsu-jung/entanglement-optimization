...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(
        description="Submit GPU jobs to resume sequential relaxation on large entangled packings."
    )
    p.add_argument("--base-dir", type=str, default="relaxation_gpu_large_N",
                   help="Base directory containing the Nxxxx/k1,k2,k3 subfolders.")
    ...
    p.add_argument("--partition", type=str, default="gpu_test",
                   help="SLURM partition (default: gpu_test).")
...
def generate_sbatch(args: argparse.Namespace, target_dir: str) -> str:
    # Resolve absolute path for the script call inside the target dir
    return f"""#!/bin/bash
#SBATCH -n 1
#SBATCH -c 1
#SBATCH -N 1
...
def main() -> None:
    ...
    print(f"Found {len(target_dirs)} packing(s) to resume relaxation on.")
